...
const ecgElement = document.getElementById("ecg_");
const bpmElement = document.getElementById("bpm_");
  ...
  if (user) {
    ...
    chartRef.on('value', snapshot =>{
      ...
      dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
        ...
        var electrocardiogram = jsonData.ECG;
        var timestamp = jsonData.timestamp;
        bpmElement.innerHTML = jsonData.BPM;
        // Plot the values on the charts
        plotValues(chartT, timestamp, electrocardiogram);
      });
    });
    ...
    dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
      ...
      var electrocardiogram = jsonData.ECG;
      var heartrate = jsonData.BPM;
      var timestamp = jsonData.timestamp;
      // Update DOM elements
      tempElement.innerHTML = temperature;
      humElement.innerHTML = humidity;
      presElement.innerHTML = pressure;
      ecgElement.innerHTML = electrocardiogram;
      bpmElement.innerHTML = heartrate;
    ...
    function createTable(){
      ...
      dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
        if (snapshot.exists()) {
          ...
          var heartrate = jsonData.BPM;
          var heartpressure = jsonData.ECG;
          var temperature = jsonData.temperature;
          var humidity = jsonData.humidity;
          var timestamp = jsonData.timestamp;
          bpmElement.innerHTML = heartrate;
        ...
        if (snapshot.exists()) {
          ...
          reversedList.forEach(element =>{
            if (firstTime){ // ignore first reading (it's already on the table from the previous query)
              ...
              bpmElement.innerHTML = heartrate;
              var heartpressure = element.heartpressure;
              var temperature = jsonData.temperature;
              var humidity = jsonData.humidity;
